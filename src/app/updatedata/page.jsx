...
export default function UpdateMahasiswa() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq("nim", nim); // Update berdasarkan NIM
  ...
  return (
    <div className="max-w-md mx-auto mt-10">
      ...
      <form onSubmit={handleUpdate} className="space-y-6 bg-white p-8 rounded-lg shadow-lg">
        <div className="space-y-2">
          ...
          <input
            ...
            placeholder="Masukkan NIM (Wajib untuk Update)"
          />
        </div>
        <div className="space-y-2">
        ...
        <div className="space-y-2">
          ...
          <CldUploadWidget uploadPreset="wubnqevu"
            onSuccess={(result) => {
          ...
          {foto && (
            <p className="text-sm text-green-600 font-medium flex items-center">
              <svg className="w-4 h-4 mr-1 fill-none stroke-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
        ...
        <button
          type="submit"
          className="w-full bg-yellow-500 text-white py-3 px-6 rounded-md hover:bg-yellow-600 transition duration-200 font-medium shadow-sm mt-4"
        >
          Update Data
        </button>
      </form>
